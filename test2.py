...
class State(TypedDict):
    ...
    user_feedback: str  # Renamed from "feedback" to "user_feedback" for consistency
...
def get_user_requirement(state: State):
    ...
    st.session_state.is_reviewing = True  # Start review process
    return {"content": result.content}

def review_by_user(state: State):
    # Use session state to retrieve or initialize review status and feedback
    if st.session_state.user_review_status is None:
        st.session_state.user_review_status = "feedback"  # Default to "feedback" if not set
    
    # Form to collect review status and feedback
    with st.form(key="review_form"):
        st.write("Review the generated response:")
        st.write(st.session_state.content)
        
        # Radio button for review status
        user_review_status = st.radio("Enter review status (Accepted/Feedback):", 
                                    ["accepted", "feedback"],
                                    index=0 if st.session_state.user_review_status == "accepted" else 1,
                                    key="review_status_form")
        
        # Show text area for feedback if "feedback" is selected
        user_feedback = st.session_state.user_feedback
        if user_review_status == "feedback":
            user_feedback = st.text_area("Provide feedback details:",
                                       value=st.session_state.user_feedback,
                                       placeholder="Enter your suggestions or modifications needed...",
                                       key="feedback_input_form")
        
        # Submit button to process the review
        if st.form_submit_button("Submit Review"):
            st.session_state.user_review_status = user_review_status
            st.session_state.user_feedback = user_feedback if user_review_status == "feedback" else ""
            
            # Return the state updates for the workflow
            return {
                "user_review_status": user_review_status,
                "user_feedback": user_feedback if user_review_status == "feedback" else ""
            }
    
    # If no submission yet, return current state (or default to feedback as in your example)
    return {
...
def create_design_document(state: State):
    if state["user_feedback"]:
        st.session_state.documents = f"Document created based on feedback: {state['user_feedback']}"
    else:
        st.session_state.documents = "Document created."
    return {"documents": st.session_state.documents}

def generate_code(state: State):
    st.session_state.codes = "Code generated."
    return {"codes": st.session_state.codes}
...
if st.session_state.user_review_status:
    ...
    if st.session_state.user_review_status == "feedback" and st.session_state.user_feedback:
